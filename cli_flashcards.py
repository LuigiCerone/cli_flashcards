...
def load_questions(file_name: Path, known_dir: Path) -> pd.DataFrame:
    ...
    if not old_state_df.empty:
        ...
        print(f"Succesfully loaded {len(df)} questions, restored {len(old_state_df)} known questions from previous runs.\n")
    else:
...
def main(file_name: Path, known_dir: Path):
    questions_df, known_df = load_questions(file_name=file_name, known_dir=known_dir)
